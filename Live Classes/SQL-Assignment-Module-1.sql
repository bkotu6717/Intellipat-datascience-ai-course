...
-- 1. Find out the AVG selling cost for packages developed in Pascal
    SELECT 
        SUM(SCOST*SOLD)/SUM(SOLD) AS AVG_SEL_COST 
    FROM SOFTWARE 
...
-- 2. Display Names, Ages of all Programmers.
    SELECT 
        PNAME, 
        DOB, 
        CONCAT(CAST (DATEDIFF(year, DOB, GETDATE()) AS varchar), ' Years') as age 
    FROM PROGRAMMER
