...
-- 5. List each PROFIT with the number of Programmers having that PROF and the number of the packages in that PROF.
    ...
    SELECT PRG_PRF.PRF,
    COUNT(PRG_PRF.PGMR) AS NO_OF_PROGRAMMERS,
    SUM( 
        CASE
            WHEN S.TITLE IS NULL THEN 0
            ELSE 1
        END
     ) AS NO_OF_PACKAGES_DEVELOPED,
    (SUM(S.SCOST*S.SOLD) - SUM(S.DCOST)) AS PROFIT
    FROM PRG_PRF
    LEFT JOIN SOFTWARE S ON PRG_PRF.PGMR = S.PNAME
    WHERE s.DEVELOPIN = PRG_PRF.PRF
    GROUP BY PRG_PRF.PRF
